---
title: Home
hide:
  - toc
---

<div class="wf-hero" markdown>
<div class="wf-hero__portrait" markdown>
![Portrait of Jon Wakefield](images/photos/wakefield-portrait-5x4_1.png)
</div>
<div class="wf-hero__body" markdown>

# Jon Wakefield

<div class="wf-hero__title" markdown>[Eddie Goldenberg Research Chair](https://science.ubc.ca/news/2026-08/eddie-goldenberg-research-chairs-brings-three-world-leading-researchers-ubc-science), University of British Columbia</div>

<p class="wf-hero__meta" markdown>
**Department** &nbsp; Statistics &nbsp;|&nbsp; **University of British Columbia, Vancouver**<br>
**Find me on** &nbsp; [Google Scholar](https://scholar.google.ca/citations?hl=en&user=bM2nnC8AAAAJ) · [arXiv](https://arxiv.org/search/stat?searchtype=author&query=Wakefield,+J)
</p>

</div>
</div>

## Research interests

I am a statistician working at the intersection of methodology and global health, with particular interests in:

- Spatial demography — see the [Impact](spatial-demography.md) page
- Small area estimation
- Spatial epidemiology
- Space–time models for infectious disease data
- Estimating under-five mortality in low- and middle-income countries
- Hierarchical models for survey data
- Estimating national and subnational disease burden
- Ecological inference for non-infectious and infectious disease data
- The links between Bayesian and frequentist procedures

## Explore the site

<ul class="wf-links">
<li><a href="research/">Research and publications</a></li>
<li><a href="people/">People — students, post docs, collaborators</a></li>
<li><a href="software/">Software</a></li>
<li><a href="teaching/">Teaching</a></li>
<li><a href="spatial-demography/">Impact</a></li>
<li><a href="stab/">STAB Research Group</a></li>
<li><a href="personal/">Personal</a></li>
</ul>
